...
public class CustomGraphSearch implements SearchObject {
	...
	public ArrayList<SearchNode> search(Problem p) {
		...
		while(!frontier.isEmpty()){
			...
			SearchNode curNode = frontier.peekAtFront();
			
			if (p.isGoalState(curNode.getState())){
				path = curNode.getPathFromRoot();
				return path;
			}

			frontier.removeFirst();
			explored.add(curNode);
			ArrayList<GridPos> childStates = p.getReachableStatesFrom(curNode.getState());
